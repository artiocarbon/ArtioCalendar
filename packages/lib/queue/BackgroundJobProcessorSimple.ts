...
export class BackgroundJobProcessor {
  ...
  addJob<T>(job: Omit<BackgroundJob<T>, 'attempts' | 'createdAt'>): string {
    ...
    console.log(`Job added to queue: ${fullJob.id} (${fullJob.type})`);
  ...
  private async processJob(job: BackgroundJob): Promise<void> {
    ...
    try {
      ...
      if (job.attempts < job.maxAttempts) {
        ...
        console.error(`Job failed permanently: ${job.id} after ${job.maxAttempts} attempts`);
      }
    } finally {
...
export const EmailJobHandler: JobHandler<{
  ...
  handler: async (job) => {
    ...
    console.log(`Sending email to ${job.data.to.join(', ')}: ${job.data.subject}`);
...
export const WebhookJobHandler: JobHandler<{
  ...
  type: 'webhook',
  handler: async (job) => {
    // Mock webhook sending for testing
    console.log(`Sending webhook to ${job.data.url}`);
...
export const AnalyticsJobHandler: JobHandler<{
  ...
  type: 'analytics',
  handler: async (job) => {
    // Mock analytics tracking for testing
    console.log(`Tracking analytics event: ${job.data.event}`, job.data.properties);
    
    // Simulate analytics tracking delay
